...
router.get('/conversations', authenticate, async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    logger.error('GET /chat/conversations error:', err);
...
router.post('/conversations', authenticate, async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    logger.error('POST /chat/conversations error:', err);
    res.status(500).json({ error: 'Помилка сервера' });
  }
});
...
router.get('/:id/messages', authenticate, async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    logger.error('GET /chat/:id/messages error:', err);
    res.status(500).json({ error: 'Помилка сервера' });
  }
});
...
router.post('/:id/messages', authenticate, async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    logger.error('POST /chat/:id/messages error:', err);
    res.status(500).json({ error: 'Помилка сервера' });
  }
});
