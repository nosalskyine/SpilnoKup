...
export async function verifyOtp(phone, otp, name, city, mode) {
  const data = await request('/auth/verify-otp', { method: 'POST', body: JSON.stringify({ phone, otp, name, city, mode }) });
  if (data.accessToken) {
