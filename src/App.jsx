...
function Nav({ tab, setTab }) {
  ...
  return (
    <div style={{ position:"absolute", bottom:0, left:0, right:0, height:64,
      background:"#0a0a0a", borderTop:"1px solid #1a1a1a", display:"flex", zIndex:100 }}>
...
function Tabs({ items, active, onChange }) {
  return (
    <div style={{display:"flex",background:"#111",borderRadius:14,padding:4,margin:"0 16px 20px"}}>
      {items.map(([id,label])=>(
  ...
  return (
    <div onClick={()=>onOpen(deal)} style={{
      ...
      <div style={{display:"flex",gap:6,padding:"10px 14px 0"}}>
        {deal.hot&&<span style={{background:"#f97316",color:"#fff",fontSize:10,fontWeight:800,padding:"3px 8px",borderRadius:8}}>🔥 ГАРЯЧЕ</span>}
        <span style={{marginLeft:"auto",background:"#16a34a22",color:"#22c55e",fontSize:11,fontWeight:800,padding:"3px 8px",borderRadius:8}}>-{d}%</span>
      </div>
      <div style={{padding:"10px 14px 14px"}}>
        <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:10}}>
          <div style={{fontSize:22,width:36,height:36,background:"#111",borderRadius:10,display:"flex",alignItems:"center",justifyContent:"center"}}>{deal.avatar}</div>
          <div>
            <div style={{fontSize:12,fontWeight:700,color:"#e5e5e5"}}>{deal.seller}</div>
            <div style={{fontSize:10,color:"#666"}}>📍{deal.city} · ⭐{deal.rating}</div>
          </div>
          {isIn&&<div style={{marginLeft:"auto",width:28,height:28,background:"#22c55e",borderRadius:"50%",display:"flex",alignItems:"center",justifyContent:"center",color:"#fff",fontSize:14}}>✓</div>}
        </div>
        <div style={{fontSize:15,fontWeight:800,color:"#fff",marginBottom:8,lineHeight:1.3}}>{deal.title}</div>
        <div style={{display:"flex",alignItems:"baseline",gap:8,marginBottom:12}}>
          <span style={{fontSize:22,fontWeight:900,color:"#22c55e"}}>₴{deal.group}</span>
          <span style={{fontSize:12,color:"#555",textDecoration:"line-through"}}>₴{deal.retail}</span>
        ...
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
          <span style={{fontSize:11,color:"#666"}}>⏰ {deal.days} {deal.days===1?"день":"дні"}</span>
          <button onClick={e=>{e.stopPropagation();onJoin(deal.id);}} style={{
            background: isIn?"#16a34a":"#22c55e", color:"#fff", border:"none",
...
function MarketPage({ joined, onJoin, onOpen }) {
  ...
  return (
    <div>
      <div style={{background:"linear-gradient(135deg,#052e16,#14532d)",padding:"24px 16px 20px",marginBottom:16}}>
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:16}}>
          <div>
            <div style={{fontSize:24,fontWeight:900,color:"#fff",letterSpacing:-1}}>СпільноКуп</div>
            <div style={{fontSize:11,color:"#86efac"}}>Малий бізнес — великі можливості</div>
          </div>
          <div style={{background:"rgba(34,197,94,0.2)",border:"1px solid #22c55e44",borderRadius:12,padding:"8px 12px",textAlign:"center"}}>
            <div style={{fontSize:16,fontWeight:900,color:"#22c55e"}}>₴4 280 000</div>
            <div style={{fontSize:9,color:"#86efac"}}>зекономлено разом</div>
          </div>
        </div>
      ...
      <div style={{padding:"0 16px 12px"}}>
        <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="🔍 Пошук: курчата, мед, сир..."
          style={{width:"100%",background:"#1a1a1a",border:"1px solid #2a2a2a",borderRadius:14,padding:"12px 16px",color:"#fff",fontSize:14,boxSizing:"border-box",outline:"none"}}/>
      </div>
      ...
      <div style={{padding:"0 16px 80px",display:"flex",flexDirection:"column",gap:12}}>
        ...
        {list.length===0&&<div style={{textAlign:"center",padding:"60px 0",color:"#555"}}><div style={{fontSize:48}}>🔍</div><div style={{marginTop:12}}>Нічого не знайдено</div></div>}
      </div>
    </div>
  );
}
...
function DealDetail({ deal, onBack, joined, onJoin, onBuy }) {
  ...
  return (
    <div style={{paddingBottom:100}}>
      <div style={{background:"linear-gradient(180deg,#0a1a0f,#0f0f0f)",padding:"20px 16px 24px"}}>
        <button onClick={onBack} style={{background:"none",border:"none",color:"#22c55e",fontSize:14,cursor:"pointer",marginBottom:16,padding:0,fontWeight:700}}>← Назад</button>
        <div style={{display:"flex",gap:8,marginBottom:12}}>
          {deal.hot&&<span style={{background:"#f97316",color:"#fff",fontSize:10,fontWeight:800,padding:"4px 10px",borderRadius:8}}>🔥 ГАРЯЧЕ</span>}
          <span style={{background:"#16a34a22",color:"#22c55e",fontSize:11,fontWeight:800,padding:"4px 10px",borderRadius:8}}>-{d}%</span>
        </div>
        <h1 style={{fontSize:22,fontWeight:900,color:"#fff",margin:"0 0 16px",lineHeight:1.3}}>{deal.title}</h1>
        <div style={{background:"#0d1f14",border:"1px solid #1a3a24",borderRadius:14,padding:14,display:"flex",gap:12,alignItems:"center"}}>
          <div style={{fontSize:36,width:52,height:52,background:"#0a1a0f",borderRadius:14,display:"flex",alignItems:"center",justifyContent:"center"}}>{deal.avatar}</div>
          <div style={{flex:1}}>
            <div style={{fontSize:15,fontWeight:800,color:"#fff"}}>{deal.seller}</div>
            <div style={{fontSize:11,color:"#666",marginTop:2}}>📍{deal.city}</div>
            <div style={{fontSize:11,color:"#eab308",marginTop:2}}>⭐{deal.rating} · {deal.deals} угод</div>
          </div>
          <div style={{textAlign:"center"}}><div style={{fontSize:10,color:"#666",marginBottom:4}}>Перевірений</div><div style={{fontSize:20}}>✅</div></div>
        </div>
      </div>

      <div style={{padding:16,display:"flex",flexDirection:"column",gap:16}}>
        <div style={{background:"#0d1f14",border:"1px solid #1a3a24",borderRadius:16,padding:16}}>
          <div style={{display:"flex",alignItems:"baseline",gap:12,marginBottom:8}}>
            <span style={{fontSize:32,fontWeight:900,color:"#22c55e"}}>₴{deal.group}</span>
            <span style={{fontSize:16,color:"#555",textDecoration:"line-through"}}>₴{deal.retail}</span>
            <span style={{fontSize:14,color:"#888"}}>/ {deal.unit}</span>
          </div>
          <div style={{fontSize:13,color:"#86efac"}}>💰 Економія ₴{deal.retail-deal.group} на {deal.unit}</div>
        </div>

        <div style={{background:"#1a1a1a",borderRadius:16,padding:16}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}>
            <div>
              <div style={{fontSize:11,color:"#666"}}>Зібрано учасників</div>
              <div style={{fontSize:24,fontWeight:900,color:"#fff"}}>{deal.joined} <span style={{fontSize:14,color:"#555"}}>/ {deal.needed}</span></div>
            </div>
          ...
          <div style={{display:"flex",justifyContent:"space-between",fontSize:12}}>
            <span style={{color:"#666"}}>⏰ Залишилось: <span style={{color:"#fff",fontWeight:700}}>{deal.days} дн.</span></span>
            <span style={{color:col,fontWeight:700}}>{p}% зібрано</span>
          </div>
        </div>

        <div style={{display:"flex",flexWrap:"wrap",gap:8}}>
          {deal.tags.map((t,i)=><span key={i} style={{background:"#1a1a1a",color:"#888",fontSize:12,padding:"6px 12px",borderRadius:10}}>{t}</span>)}
        </div>

        <div style={{background:"#1a1a1a",borderRadius:16,padding:16}}>
          <div style={{fontSize:13,fontWeight:700,color:"#fff",marginBottom:8}}>📋 Опис</div>
          <div style={{fontSize:13,color:"#888",lineHeight:1.6}}>{deal.desc}</div>
        </div>

        {!isIn&&(
          <div style={{background:"#1a1a1a",borderRadius:16,padding:16}}>
            <div style={{fontSize:13,fontWeight:700,color:"#fff",marginBottom:12}}>📦 Кількість ({deal.unit})</div>
            <div style={{display:"flex",alignItems:"center",gap:16}}>
              <button onClick={()=>setQty(Math.max(deal.min,qty-1))} style={{width:40,height:40,borderRadius:12,background:"#111",border:"none",color:"#fff",fontSize:20,cursor:"pointer"}}>−</button>
              <span style={{fontSize:24,fontWeight:900,color:"#fff",flex:1,textAlign:"center"}}>{qty}</span>
              <button onClick={()=>setQty(Math.min(deal.max,qty+1))} style={{width:40,height:40,borderRadius:12,background:"#22c55e",border:"none",color:"#fff",fontSize:20,cursor:"pointer"}}>+</button>
            </div>
            <div style={{fontSize:12,color:"#666",marginTop:8,textAlign:"center"}}>Мін. {deal.min} · Макс. {deal.max} {deal.unit}</div>
          </div>
        )}
      </div>

      <div style={{position:"fixed",bottom:64,left:"50%",transform:"translateX(-50%)",width:358,padding:"12px 16px",background:"#0f0f0f",borderTop:"1px solid #1a1a1a",zIndex:50}}>
        {isIn?(
          <div style={{display:"flex",gap:10}}>
            <div style={{flex:1,background:"#052e16",border:"1px solid #22c55e44",borderRadius:14,padding:14,textAlign:"center"}}>
              <div style={{fontSize:14,fontWeight:800,color:"#22c55e"}}>✓ Ти в групі!</div>
              <div style={{fontSize:11,color:"#86efac",marginTop:2}}>Повідомимо коли наберуть</div>
            </div>
            <button onClick={()=>onBuy(deal,qty)} style={{background:"#6366f1",color:"#fff",border:"none",borderRadius:14,padding:"0 16px",fontSize:13,fontWeight:800,cursor:"pointer"}}>
              📱 QR
            </button>
          </div>
        ):(
          <button onClick={()=>{onJoin(deal.id);onBuy(deal,qty);}} style={{width:"100%",padding:16,background:"#22c55e",border:"none",borderRadius:14,color:"#fff",fontSize:16,fontWeight:900,cursor:"pointer"}}>
...
function MyDealsPage({ joined, onOpen }) {
  ...
  return (
    <div style={{padding:"20px 16px 80px"}}>
      <h2 style={{color:"#fff",fontSize:20,fontWeight:900,marginBottom:4}}>Мої покупки</h2>
      <p style={{color:"#666",fontSize:13,marginBottom:20}}>{my.length} активних</p>
      {my.length===0?(
        <div style={{textAlign:"center",padding:"80px 0"}}>
          <div style={{fontSize:56}}>🛒</div>
          <div style={{color:"#555",marginTop:16,fontSize:14}}>Ти ще не приєднався до жодної покупки</div>
          <div style={{color:"#444",marginTop:8,fontSize:12}}>Перейди на вкладку Маркет</div>
        </div>
      ):(
        <div style={{display:"flex",flexDirection:"column",gap:12}}>
          {my.map(d=>(
            <div key={d.id} onClick={()=>onOpen(d)} style={{background:"#1a1a1a",borderRadius:16,padding:16,cursor:"pointer",border:"1px solid #22c55e33"}}>
              <div style={{display:"flex",gap:12,alignItems:"center",marginBottom:10}}>
                <div style={{fontSize:28,width:48,height:48,background:"#111",borderRadius:12,display:"flex",alignItems:"center",justifyContent:"center"}}>{d.avatar}</div>
                <div style={{flex:1}}>
                  <div style={{fontSize:14,fontWeight:800,color:"#fff",lineHeight:1.3}}>{d.title}</div>
                  <div style={{fontSize:11,color:"#666",marginTop:2}}>{d.seller}</div>
                </div>
                <div style={{textAlign:"right"}}>
                  <div style={{fontSize:18,fontWeight:900,color:"#22c55e"}}>₴{d.group}</div>
                  <div style={{fontSize:10,color:"#888"}}>/{d.unit}</div>
                </div>
              </div>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                <div style={{flex:1,height:6,background:"#111",borderRadius:3,marginRight:12,overflow:"hidden"}}>
                  <div style={{height:"100%",width:`${pct(d)}%`,background:"#22c55e",borderRadius:3}}/>
                </div>
                <span style={{background:"#052e16",color:"#22c55e",fontSize:10,fontWeight:800,padding:"4px 10px",borderRadius:8}}>✓ В групі</span>
              </div>
            </div>
          ))}
        </div>
...
function BuyerQRPage({ order, onBack }) {
  ...
  return (
    <div style={{padding:20,display:"flex",flexDirection:"column",alignItems:"center"}}>
      <button onClick={onBack} style={{background:"none",border:"none",color:"#22c55e",fontSize:14,cursor:"pointer",padding:0,fontWeight:700,alignSelf:"flex-start"}}>← Назад</button>
      <div style={{fontSize:13,fontWeight:700,color:"#666",marginBottom:4,marginTop:12}}>Замовлення #{order.id}</div>
      <div style={{fontSize:18,fontWeight:900,color:"#fff",marginBottom:4,textAlign:"center"}}>{order.item}</div>
      ...
      <div style={{padding:20,background:"#fff",borderRadius:24,boxShadow:status==="completed"?"0 0 40px rgba(34,197,94,0.3)":"0 0 40px rgba(0,0,0,0.5)",marginBottom:20,position:"relative"}}>
        ...
        {status==="completed"&&(
          <div style={{position:"absolute",inset:0,background:"rgba(5,46,22,0.88)",borderRadius:24,display:"flex",alignItems:"center",justifyContent:"center",flexDirection:"column",gap:8}}>
            ...
            <div style={{fontSize:14,fontWeight:900,color:"#22c55e"}}>Отримано!</div>
          </div>
        )}
      </div>
      <div style={{background:"#1a1a1a",borderRadius:16,padding:16,width:"100%",boxSizing:"border-box",fontSize:12,color:"#666",textAlign:"center",lineHeight:1.6}}>
        Покажи цей QR код продавцю при отриманні товару
      </div>
      {done&&(
        <div style={{position:"fixed",top:20,left:"50%",transform:"translateX(-50%)",background:"#052e16",border:"1px solid #22c55e",borderRadius:16,padding:"14px 20px",display:"flex",gap:10,alignItems:"center",zIndex:999}}>
          <span style={{fontSize:20}}>🎉</span>
          <div><div style={{fontSize:13,fontWeight:800,color:"#22c55e"}}>Товар отримано!</div><div style={{fontSize:11,color:"#86efac"}}>Кошти зараховано продавцю</div></div>
        </div>
      )}
    </div>
  );
...
function ScannerView({ onScan, onClose, orders }) {
  ...
  if(result) return (
    <div style={{position:"absolute",inset:0,background:"rgba(0,0,0,0.96)",zIndex:200,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"flex-start",padding:"20px 20px 0",overflowY:"auto"}}>
      <div style={{background:"#111",borderRadius:24,padding:20,width:"100%",boxSizing:"border-box"}}>
        <div style={{textAlign:"center",marginBottom:20}}>
          <div style={{fontSize:48,marginBottom:8}}>✅</div>
          <div style={{fontSize:18,fontWeight:900,color:"#22c55e"}}>QR зчитано!</div>
          <div style={{fontSize:12,color:"#666",marginTop:4}}>#{result.id}</div>
        </div>
        <div style={{background:"#1a1a1a",borderRadius:14,padding:14,marginBottom:16,display:"flex",flexDirection:"column",gap:10}}>
          {[["👤 Покупець",result.buyer],["📦 Товар",result.item],["⚖️ Кількість",`${result.qty} ${result.unit}`]].map(([k,v],i)=>(
            <div key={i} style={{display:"flex",justifyContent:"space-between",fontSize:13}}><span style={{color:"#666"}}>{k}</span><span style={{color:"#fff",fontWeight:700}}>{v}</span></div>
          ))}
          <div style={{height:1,background:"#2a2a2a"}}/>
          <div style={{display:"flex",justifyContent:"space-between",fontSize:13}}><span style={{color:"#666"}}>💰 До виплати</span><span style={{color:"#22c55e",fontSize:20,fontWeight:900}}>₴{result.amount}</span></div>
        </div>
        <div style={{background:"#052e16",border:"1px solid #22c55e33",borderRadius:12,padding:12,marginBottom:20,fontSize:12,color:"#86efac",textAlign:"center"}}>
          💳 Кошти зарахуються одразу після підтвердження
        </div>
        <div style={{display:"flex",gap:10}}>
          <button onClick={onClose} style={{flex:1,padding:"14px 16px",background:"#1a1a1a",border:"1px solid #2a2a2a",borderRadius:14,color:"#888",fontSize:13,fontWeight:700,cursor:"pointer"}}>Скасувати</button>
          <button onClick={()=>onScan(result)} style={{flex:2,padding:"14px 16px",background:"#22c55e",border:"none",borderRadius:14,color:"#fff",fontSize:14,fontWeight:800,cursor:"pointer"}}>Підтвердити видачу ✓</button>
        </div>
      </div>
    </div>
  );
  return (
    <div style={{position:"absolute",inset:0,background:"rgba(0,0,0,0.96)",zIndex:200,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:20}}>
      <div style={{fontSize:18,fontWeight:800,color:"#fff",marginBottom:4}}>Скануй QR покупця</div>
      <div style={{fontSize:12,color:"#666",marginBottom:24}}>Направ камеру на QR код</div>
      <div style={{width:260,height:260,position:"relative",margin:"0 auto 24px"}}>
        <div style={{width:"100%",height:"100%",background:"#111",borderRadius:16,overflow:"hidden",position:"relative"}}>
      ...
      <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:24}}>
        ...
        <span style={{fontSize:13,color:"#22c55e"}}>Сканування...</span>
      </div>
      <button onClick={onClose} style={{padding:"12px 32px",background:"#1a1a1a",border:"1px solid #2a2a2a",borderRadius:14,color:"#888",fontSize:13,fontWeight:700,cursor:"pointer"}}>Закрити</button>
    </div>
  );
}
...
function SellerScanPage({ orders, onScanDone, onBack }) {
  ...
  return (
    <div>
      ...
      {paid&&(
        <div style={{position:"fixed",top:20,left:"50%",transform:"translateX(-50%)",background:"#052e16",border:"1px solid #22c55e",borderRadius:16,padding:"14px 20px",display:"flex",gap:10,alignItems:"center",zIndex:999,whiteSpace:"nowrap"}}>
          <span style={{fontSize:20}}>💰</span>
          <div><div style={{fontSize:13,fontWeight:800,color:"#22c55e"}}>+₴{paid.amount} зараховано!</div><div style={{fontSize:11,color:"#86efac"}}>{paid.buyer}</div></div>
        </div>
      )}
      <div style={{background:"linear-gradient(135deg,#052e16,#14532d)",padding:"24px 16px 20px"}}>
        <button onClick={onBack} style={{background:"none",border:"none",color:"#86efac",fontSize:13,cursor:"pointer",padding:0,marginBottom:12,fontWeight:700}}>← Назад</button>
        <div style={{fontSize:20,fontWeight:900,color:"#fff",marginBottom:4}}>📱 Сканер QR кодів</div>
        <div style={{fontSize:12,color:"#86efac",marginBottom:16}}>Скануй QR — отримуй гроші моментально</div>
        <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:8}}>
          {[["Очікують",pending.length,"#fbbf24"],["Видано",done.length,"#22c55e"],["Зароблено",`₴${done.reduce((s,o)=>s+o.amount,0)}`,"#86efac"]].map(([l,v,c],i)=>(
            <div key={i} style={{background:"rgba(0,0,0,0.3)",borderRadius:12,padding:"10px 8px",textAlign:"center"}}>
              ...
              <div style={{fontSize:10,color:"#86efac",marginTop:2}}>{l}</div>
            </div>
          ))}
        </div>
      </div>
      <div style={{padding:16}}>
        <button onClick={()=>setShowScan(true)} style={{width:"100%",padding:20,background:"#22c55e",border:"none",borderRadius:18,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",gap:12,boxShadow:"0 8px 32px rgba(34,197,94,0.4)"}}>
          ...
          <div style={{textAlign:"left"}}>
            <div style={{fontSize:17,fontWeight:900,color:"#fff"}}>Сканувати QR код</div>
      ...
      {pending.length>0&&(
        <div style={{padding:"0 16px 8px"}}>
          ...
          <div style={{display:"flex",flexDirection:"column",gap:10}}>
            {pending.map(o=>(
              <div key={o.id} style={{background:"#1a1a1a",borderRadius:14,padding:14,border:"1px solid #2a2a2a"}}>
                <div style={{display:"flex",gap:10,alignItems:"center"}}>
                  <div style={{width:44,height:44,background:"#111",borderRadius:12,display:"flex",alignItems:"center",justifyContent:"center",fontSize:24}}>{o.avatar}</div>
                  <div style={{flex:1}}>
                    <div style={{fontSize:14,fontWeight:800,color:"#fff"}}>{o.buyer}</div>
                    <div style={{fontSize:12,color:"#666"}}>{o.item} · {o.qty} {o.unit}</div>
                    <div style={{fontSize:11,color:"#888",marginTop:2}}>📞 {o.phone}</div>
                  </div>
                  <div style={{textAlign:"right"}}>
                    <div style={{fontSize:18,fontWeight:900,color:"#fbbf24"}}>₴{o.amount}</div>
                    <div style={{fontSize:10,color:"#666",marginTop:2}}>очікує</div>
                  </div>
                </div>
              </div>
            ))}
      ...
      {done.length>0&&(
        <div style={{padding:"8px 16px 80px"}}>
          <div style={{fontSize:13,fontWeight:800,color:"#22c55e",marginBottom:10}}>✅ Видано ({done.length})</div>
          <div style={{display:"flex",flexDirection:"column",gap:8}}>
            {done.map(o=>(
              <div key={o.id} style={{background:"#0d1a0d",borderRadius:14,padding:14,border:"1px solid #1a3a1a",opacity:0.8}}>
                <div style={{display:"flex",gap:10,alignItems:"center"}}>
                  <div style={{width:36,height:36,background:"#052e16",borderRadius:10,display:"flex",alignItems:"center",justifyContent:"center",fontSize:20}}>{o.avatar}</div>
                  <div style={{flex:1}}>
                    <div style={{fontSize:13,fontWeight:700,color:"#86efac"}}>{o.buyer}</div>
                    <div style={{fontSize:11,color:"#555"}}>{o.item} · {o.qty} {o.unit}</div>
                  </div>
                  <div style={{textAlign:"right"}}>
                    <div style={{fontSize:16,fontWeight:900,color:"#22c55e"}}>+₴{o.amount}</div>
  ...
  return (
    <div style={{padding:24,display:"flex",flexDirection:"column",height:"100%",boxSizing:"border-box"}}>
      <div style={{textAlign:"center",marginBottom:36,marginTop:20}}>
        ...
        <h1 style={{fontSize:24,fontWeight:900,color:"#fff",margin:"0 0 8px"}}>QR Видача</h1>
        <p style={{fontSize:13,color:"#666",margin:0}}>Система підтвердження отримання товарів</p>
      </div>
      <div style={{display:"flex",flexDirection:"column",gap:14,marginBottom:32}}>
        <button onClick={()=>setMode("buyer")} style={{background:"#1a1a1a",border:"1px solid #2a2a2a",borderRadius:20,padding:20,cursor:"pointer",textAlign:"left",display:"flex",gap:16,alignItems:"center"}}>
          <div style={{width:56,height:56,background:"#0d1f14",borderRadius:16,display:"flex",alignItems:"center",justifyContent:"center",fontSize:28}}>📱</div>
          <div><div style={{fontSize:17,fontWeight:800,color:"#fff",marginBottom:4}}>Я — Покупець</div><div style={{fontSize:12,color:"#666",lineHeight:1.5}}>Показую QR код для отримання товару</div></div>
        </button>
        <button onClick={()=>setMode("seller")} style={{background:"#1a1a1a",border:"1px solid #2a2a2a",borderRadius:20,padding:20,cursor:"pointer",textAlign:"left",display:"flex",gap:16,alignItems:"center"}}>
          <div style={{width:56,height:56,background:"#0d1a2e",borderRadius:16,display:"flex",alignItems:"center",justifyContent:"center",fontSize:28}}>📷</div>
          <div><div style={{fontSize:17,fontWeight:800,color:"#fff",marginBottom:4}}>Я — Продавець</div><div style={{fontSize:12,color:"#666",lineHeight:1.5}}>Сканую QR і підтверджую видачу</div></div>
        </button>
      </div>
      <div style={{background:"#1a1a1a",borderRadius:18,padding:18}}>
        ...
        {[["💳","Покупець оплачує онлайн"],["📱","Отримує унікальний QR"],["📷","Продавець сканує при видачі"],["💰","Гроші миттєво на рахунок"]].map(([icon,text],i)=>(
          <div key={i} style={{display:"flex",gap:12,alignItems:"center",marginBottom:i<3?12:0}}>
            <div style={{width:32,height:32,background:"#111",borderRadius:10,display:"flex",alignItems:"center",justifyContent:"center",fontSize:16}}>{icon}</div>
    ...
    <div>
      <div style={{background:"linear-gradient(135deg,#1e1b4b,#312e81)",padding:"24px 16px 20px"}}>
        <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:16}}>
          ...
          <div>
            <div style={{fontSize:18,fontWeight:900,color:"#fff"}}>{SELLER_PROFILE.name}</div>
      ...
      <div style={{padding:16}}>
        {tab==="dash"&&(
          <div style={{display:"flex",flexDirection:"column",gap:12}}>
            <div style={{background:"#1a1a1a",borderRadius:16,padding:16}}>
              <div style={{fontSize:13,fontWeight:700,color:"#fff",marginBottom:12}}>📈 Продажі за тиждень</div>
              <div style={{display:"flex",alignItems:"flex-end",gap:6,height:80}}>
                {[40,65,45,80,55,90,70].map((h,i)=>(
                  <div key={i} style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",gap:4}}>
                    <div style={{width:"100%",height:h*0.8,background:i===5?"#22c55e":"#2a2a2a",borderRadius:4}}/>
                    <div style={{fontSize:9,color:"#555"}}>{"ПнВтСрЧтПтСбНд".slice(i*2,i*2+2)}</div>
                  </div>
                ))}
              </div>
            </div>
            {[["💰","Заробіток сьогодні","₴3,840","#22c55e"],["👥","Нових учасників","+12","#6366f1"],["⏳","Активних угод","3","#f97316"],["⭐","Нових відгуків","5","#eab308"]].map(([ic,l,v,c],i)=>(
              <div key={i} style={{background:"#1a1a1a",borderRadius:14,padding:"14px 16px",display:"flex",alignItems:"center",gap:12}}>
                <span style={{fontSize:24}}>{ic}</span>
                <div style={{flex:1}}><div style={{fontSize:11,color:"#666"}}>{l}</div><div style={{fontSize:18,fontWeight:900,color:c}}>{v}</div></div>
              </div>
            ))}
            <button onClick={onScanQR} style={{width:"100%",padding:16,background:"#052e16",border:"1px solid #22c55e44",borderRadius:14,color:"#22c55e",fontSize:14,fontWeight:800,cursor:"pointer"}}>
        ...
        {tab==="listings"&&(
          <div style={{display:"flex",flexDirection:"column",gap:12}}>
            <button onClick={onCreateDeal} style={{background:"#22c55e",color:"#fff",border:"none",borderRadius:14,padding:16,fontSize:15,fontWeight:800,cursor:"pointer"}}>+ Створити нову пропозицію</button>
            {listings.map(d=>(
              <div key={d.id} style={{background:"#1a1a1a",borderRadius:14,padding:14,border:"1px solid #2a2a2a"}}>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:8}}>
                  <div style={{fontSize:14,fontWeight:700,color:"#fff",flex:1}}>{d.title}</div>
                ...
                <div style={{marginTop:8,height:4,background:"#111",borderRadius:2,overflow:"hidden"}}>
        ...
        {tab==="orders"&&(
          <div style={{display:"flex",flexDirection:"column",gap:10}}>
            {[
              ...
              <div key={i} style={{background:"#1a1a1a",borderRadius:14,padding:14,display:"flex",gap:12,alignItems:"center"}}>
                <div style={{width:40,height:40,background:"#111",borderRadius:12,display:"flex",alignItems:"center",justifyContent:"center",fontSize:18}}>👤</div>
                <div style={{flex:1}}>
                  <div style={{fontSize:13,fontWeight:700,color:"#fff"}}>{o.name}</div>
                  <div style={{fontSize:11,color:"#666"}}>{o.item} · {o.time}</div>
...
function CreateDeal({ onBack }) {
  ...
  if(step===3) return (
    <div style={{padding:24,textAlign:"center",paddingTop:80}}>
      ...
      <h2 style={{color:"#fff",fontSize:24,fontWeight:900,margin:"16px 0 8px"}}>Опубліковано!</h2>
      <p style={{color:"#666",fontSize:13,lineHeight:1.7}}>Люди вже бачать твій товар!</p>
      <div style={{background:"#1a1a1a",borderRadius:14,padding:16,margin:"24px 0",display:"flex",gap:12,alignItems:"center"}}>
        <div style={{fontSize:32}}>📦</div>
        <div style={{textAlign:"left"}}>
          <div style={{fontSize:14,fontWeight:700,color:"#fff"}}>{form.title||"Новий товар"}</div>
          <div style={{fontSize:12,color:"#22c55e"}}>₴{form.group}/{form.unit||"шт"} · -{dsc}%</div>
        </div>
      </div>
      <button onClick={onBack} style={{width:"100%",padding:16,background:"#22c55e",border:"none",borderRadius:14,color:"#fff",fontSize:15,fontWeight:800,cursor:"pointer"}}>На Бізнес панель</button>
    </div>
  );
  return (
    <div style={{padding:16}}>
      <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:24}}>
        <button onClick={onBack} style={{background:"none",border:"none",color:"#22c55e",fontSize:20,cursor:"pointer"}}>←</button>
        <h2 style={{color:"#fff",fontSize:18,fontWeight:900,margin:0}}>Нова пропозиція</h2>
      ...
      {step===1&&(
        <div style={{display:"flex",flexDirection:"column",gap:16}}>
          ...
          <div>
            <label style={{fontSize:12,color:"#666",display:"block",marginBottom:6}}>📦 Категорія</label>
            <div style={{display:"flex",flexWrap:"wrap",gap:8}}>
          ...
          {[{k:"title",l:"📝 Назва",p:"напр. Курчата бройлер"},{k:"unit",l:"⚖️ Одиниця",p:"кг, шт, лоток..."},{k:"desc",l:"📄 Опис",p:"Розкажи про якість і умови..."}].map(f=>(
            <div key={f.k}>
              <label style={{fontSize:12,color:"#666",display:"block",marginBottom:6}}>{f.l}</label>
              {f.k==="desc"
                ?<textarea value={form[f.k]} onChange={e=>set(f.k,e.target.value)} placeholder={f.p} rows={3} style={{width:"100%",background:"#1a1a1a",border:"1px solid #2a2a2a",borderRadius:12,padding:"12px 14px",color:"#fff",fontSize:14,boxSizing:"border-box",outline:"none",resize:"none"}}/>
                :<input value={form[f.k]} onChange={e=>set(f.k,e.target.value)} placeholder={f.p} style={{width:"100%",background:"#1a1a1a",border:"1px solid #2a2a2a",borderRadius:12,padding:"12px 14px",color:"#fff",fontSize:14,boxSizing:"border-box",outline:"none"}}/>
              }
            </div>
          ))}
          <button onClick={()=>form.title&&form.unit&&setStep(2)} style={{background:form.title&&form.unit?"#22c55e":"#2a2a2a",color:"#fff",border:"none",borderRadius:14,padding:16,fontSize:15,fontWeight:800,cursor:"pointer"}}>Далі →</button>
        </div>
      )}
      {step===2&&(
        <div style={{display:"flex",flexDirection:"column",gap:16}}>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
            {[{k:"retail",l:"🏪 Ціна в магазині (₴)",p:"95"},{k:"group",l:"🤝 Оптова ціна (₴)",p:"68"}].map(f=>(
              <div key={f.k}>
                <label style={{fontSize:12,color:"#666",display:"block",marginBottom:6}}>{f.l}</label>
                <input type="number" value={form[f.k]} onChange={e=>set(f.k,e.target.value)} placeholder={f.p} style={{width:"100%",background:"#1a1a1a",border:"1px solid #2a2a2a",borderRadius:12,padding:"12px 14px",color:"#fff",fontSize:14,boxSizing:"border-box",outline:"none"}}/>
              </div>
            ))}
          </div>
          {dsc>0&&<div style={{background:"#052e16",border:"1px solid #22c55e44",borderRadius:12,padding:14,display:"flex",alignItems:"center",gap:12}}><div style={{fontSize:28}}>🎯</div><div><div style={{fontSize:11,color:"#86efac"}}>Покупці побачать знижку</div><div style={{fontSize:22,fontWeight:900,color:"#22c55e"}}>-{dsc}%</div></div></div>}
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
          ...
          <button onClick={()=>form.retail&&form.group&&form.needed&&setStep(3)} style={{background:form.retail&&form.group&&form.needed?"#22c55e":"#2a2a2a",color:"#fff",border:"none",borderRadius:14,padding:16,fontSize:15,fontWeight:800,cursor:"pointer"}}>Опублікувати 🚀</button>
        </div>
      )}
    </div>
  );
  ...
  return (
    <div>
      <div style={{margin:"0 16px 20px",background:"linear-gradient(135deg,#0a1628,#1e3a5f,#0a1628)",borderRadius:24,padding:24,position:"relative",overflow:"hidden"}}>
        ...
        <div style={{fontSize:38,fontWeight:900,color:"#fff",letterSpacing:-1,marginBottom:4}}>₴3 380,32</div>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
          <div style={{background:"rgba(255,255,255,0.06)",borderRadius:14,padding:12}}>
            ...
            <div style={{fontSize:20,fontWeight:900,color:"#22c55e"}}>₴2 847,20</div>
          </div>
          <div style={{background:"rgba(255,255,255,0.06)",borderRadius:14,padding:12}}>
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,margin:"0 16px 20px"}}>
        <button onClick={onWithdraw} style={{background:"#22c55e",border:"none",borderRadius:16,padding:"16px 12px",cursor:"pointer",display:"flex",flexDirection:"column",alignItems:"center",gap:6}}>
          ...
          <span style={{fontSize:13,fontWeight:800,color:"#fff"}}>Вивести кошти</span>
          <span style={{fontSize:10,color:"rgba(255,255,255,0.7)"}}>На IBAN рахунок</span>
        </button>
        <button style={{background:"#1a1a1a",border:"1px solid #2a2a4a",borderRadius:16,padding:"16px 12px",cursor:"pointer",display:"flex",flexDirection:"column",alignItems:"center",gap:6}}>
          <span style={{fontSize:24}}>📊</span>
          <span style={{fontSize:13,fontWeight:800,color:"#fff"}}>Статистика</span>
          <span style={{fontSize:10,color:"#666"}}>Доходи і витрати</span>
        </button>
      </div>
      <div style={{margin:"0 16px 20px",background:"#1a1a1a",borderRadius:18,padding:16}}>
        <div style={{fontSize:13,fontWeight:800,color:"#fff",marginBottom:14}}>📈 Березень 2024</div>
        <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:8,marginBottom:14}}>
          {[["Дохід","₴12 840","#22c55e"],["Комісія","₴513","#f87171"],["Чистий","₴12 327","#60a5fa"]].map(([l,v,c],i)=>(
            <div key={i} style={{background:"#111",borderRadius:12,padding:"10px 8px",textAlign:"center"}}>
        ...
        <div style={{fontSize:10,color:"#555",marginTop:6,textAlign:"center"}}>Доходи за 14 днів</div>
      </div>
      <div style={{margin:"0 16px 80px"}}>
        <div style={{fontSize:13,fontWeight:800,color:"#fff",marginBottom:12}}>🧾 Останні операції</div>
        <div style={{display:"flex",flexDirection:"column",gap:8}}>
          {TRANSACTIONS.map(t=>(
            <div key={t.id} style={{background:"#1a1a1a",borderRadius:14,padding:14,display:"flex",gap:12,alignItems:"center"}}>
              ...
              <div style={{flex:1}}>
                ...
                <div style={{fontSize:10,color:"#555",marginTop:3}}>{t.date}</div>
              </div>
              <div style={{textAlign:"right"}}>
...
function WithdrawPage({ onBack }) {
  ...
  if(done) return (
    <div style={{padding:24}}>
      <button onClick={onBack} style={{background:"none",border:"none",color:"#22c55e",fontSize:14,cursor:"pointer",padding:0,fontWeight:700}}>← Назад</button>
      <div style={{textAlign:"center",marginTop:20,marginBottom:32}}>
        <div style={{fontSize:56,marginBottom:16}}>✅</div>
        <div style={{fontSize:22,fontWeight:900,color:"#fff",marginBottom:8}}>Заявку подано!</div>
        <div style={{fontSize:13,color:"#666",lineHeight:1.6}}>Кошти надійдуть протягом 1-2 робочих днів</div>
      </div>
      <div style={{background:"#1a1a1a",borderRadius:18,padding:18,marginBottom:20}}>
        {[["💳 Рахунок",`${SELLER_PROFILE.iban.slice(0,10)}...${SELLER_PROFILE.iban.slice(-4)}`],["🏦 Банк",SELLER_PROFILE.bank],["💰 Сума",`₴${parseFloat(amount).toLocaleString()}`],["📊 Комісія (1%)",`-₴${fee}`],["✅ Отримаєш",`₴${recv}`]].map(([k,v],i)=>(
          <div key={i} style={{display:"flex",justifyContent:"space-between",padding:"10px 0",borderBottom:i<4?"1px solid #222":"none",fontSize:13}}>
            <span style={{color:"#666"}}>{k}</span><span style={{color:"#fff",fontWeight:700}}>{v}</span>
          </div>
        ))}
      </div>
      <button onClick={onBack} style={{width:"100%",padding:16,background:"#22c55e",border:"none",borderRadius:14,color:"#fff",fontSize:15,fontWeight:800,cursor:"pointer"}}>Повернутись до гаманця</button>
    </div>
  );
  return (
    <div style={{padding:20}}>
      <button onClick={onBack} style={{background:"none",border:"none",color:"#22c55e",fontSize:14,cursor:"pointer",padding:0,fontWeight:700}}>← Назад</button>
      <h2 style={{color:"#fff",fontSize:20,fontWeight:900,margin:"16px 0 4px"}}>Вивести кошти</h2>
      <p style={{color:"#666",fontSize:13,margin:"0 0 24px"}}>Доступно: <span style={{color:"#22c55e",fontWeight:800}}>₴{avail.toFixed(2)}</span></p>
      <div style={{background:"#1a1a1a",borderRadius:16,padding:16,marginBottom:20}}>
        <div style={{fontSize:11,color:"#666",marginBottom:8}}>Рахунок отримувача</div>
        <div style={{display:"flex",alignItems:"center",gap:12}}>
          <div style={{width:40,height:40,background:"#0d1a2e",borderRadius:12,display:"flex",alignItems:"center",justifyContent:"center",fontSize:20}}>🏦</div>
          <div>
            <div style={{fontSize:13,fontWeight:700,color:"#fff"}}>{SELLER_PROFILE.fop}</div>
            <div style={{fontSize:11,color:"#60a5fa"}}>{SELLER_PROFILE.iban.slice(0,16)}...</div>
          </div>
          <div style={{marginLeft:"auto",background:"#052e16",color:"#22c55e",fontSize:10,padding:"4px 8px",borderRadius:8,fontWeight:700}}>✓ Верифіковано</div>
        </div>
      </div>
      <div style={{marginBottom:20}}>
        <div style={{fontSize:12,color:"#666",marginBottom:8}}>Сума виведення (₴)</div>
        <div style={{position:"relative"}}>
          <span style={{position:"absolute",left:16,top:"50%",transform:"translateY(-50%)",fontSize:20,fontWeight:900,color:"#555"}}>₴</span>
          <input type="number" value={amount} onChange={e=>setAmount(e.target.value)} placeholder="0.00"
            style={{width:"100%",background:"#1a1a1a",border:"1px solid #2a2a2a",borderRadius:14,padding:"16px 16px 16px 40px",color:"#fff",fontSize:24,fontWeight:900,boxSizing:"border-box",outline:"none"}}/>
        </div>
        <div style={{display:"flex",gap:8,marginTop:10}}>
          {[500,1000,2000,avail].map((v,i)=>(
            <button key={i} onClick={()=>setAmount(v.toString())} style={{flex:1,padding:"8px 0",background:"#1a1a1a",border:"1px solid #2a2a2a",borderRadius:10,color:"#888",fontSize:11,fontWeight:700,cursor:"pointer"}}>{i===3?"Все":`₴${v}`}</button>
          ))}
        </div>
      </div>
      {amount&&parseFloat(amount)>0&&(
        <div style={{background:"#0d1a2e",border:"1px solid #1e3a5f",borderRadius:14,padding:14,marginBottom:20}}>
      ...
      <button onClick={()=>amount&&parseFloat(amount)>0&&setDone(true)}
        style={{width:"100%",padding:16,background:amount&&parseFloat(amount)>0?"#22c55e":"#2a2a2a",border:"none",borderRadius:14,color:"#fff",fontSize:15,fontWeight:800,cursor:"pointer",opacity:amount&&parseFloat(amount)>0?1:0.5}}>
        Вивести ₴{amount||"0"} →
      </button>
    </div>
  );
  ...
  return (
    <div style={{padding:"0 16px"}}>
        ...
        <div style={{background:"#1c1400",border:"1px solid #fbbf2433",borderRadius:14,padding:14,marginBottom:16}}>
          ...
          {[{date:"31.03",desc:"Єдиний податок · ₴1,600",done:false},{date:"31.03",desc:"ЄСВ за Q1 · ₴4,290",done:false},{date:"28.02",desc:"Єдиний податок · ₴1,600",done:true}].map((t,i)=>(
            <div key={i} style={{display:"flex",gap:10,alignItems:"center",padding:"6px 0",borderBottom:i<2?"1px solid #2a1a00":"none"}}>
              <div style={{width:32,textAlign:"center"}}>{t.done?<span style={{color:"#22c55e",fontSize:16}}>✓</span>:<span style={{color:"#fbbf24",fontSize:16}}>⏰</span>}</div>
              <div style={{flex:1}}>
      ...
      <div style={{display:"flex",flexDirection:"column",gap:10,marginBottom:80}}>
        {filtered.map(doc=>(
          <div key={doc.id} onClick={()=>onReceipt(doc)} style={{background:"#1a1a1a",borderRadius:14,padding:14,cursor:"pointer",border:"1px solid #2a2a2a",display:"flex",gap:12,alignItems:"center"}}>
            ...
            <div style={{flex:1}}>
              <div style={{fontSize:13,fontWeight:800,color:"#fff"}}>{doc.title}</div>
              <div style={{fontSize:11,color:"#666",marginTop:2}}>{doc.desc}</div>
              <div style={{fontSize:10,color:"#444",marginTop:2}}>📅 {doc.date}</div>
            </div>
            <div style={{textAlign:"right"}}>
              <div style={{fontSize:14,fontWeight:900,color:"#22c55e"}}>₴{doc.amount.toLocaleString()}</div>
              <div style={{fontSize:10,color:"#555",marginTop:2}}>→ PDF</div>
            </div>
          </div>
        ))}
      </div>
...
function ReceiptPage({ doc, onBack }) {
  ...
  return (
    <div style={{padding:20}}>
      <button onClick={onBack} style={{background:"none",border:"none",color:"#22c55e",fontSize:14,cursor:"pointer",padding:0,fontWeight:700}}>← Назад</button>
      <div style={{background:"#1a1a1a",borderRadius:20,padding:20,marginTop:16,marginBottom:16}}>
        <div style={{textAlign:"center",marginBottom:20,paddingBottom:20,borderBottom:"2px dashed #2a2a2a"}}>
          <div style={{fontSize:32,marginBottom:8}}>🧾</div>
          <div style={{fontSize:16,fontWeight:900,color:"#fff"}}>Платіжний чек</div>
          <div style={{fontSize:11,color:"#666",marginTop:4}}>СпільноКуп · #{doc.id}</div>
          <div style={{fontSize:11,color:"#555",marginTop:2}}>{doc.date}</div>
        </div>
        <div style={{marginBottom:16,paddingBottom:16,borderBottom:"1px solid #2a2a2a"}}>
          <div style={{fontSize:11,color:"#666",marginBottom:8,fontWeight:700}}>ПРОДАВЕЦЬ</div>
          {[["ФОП",SELLER_PROFILE.fop],["ІПН",SELLER_PROFILE.ipn],["Група",SELLER_PROFILE.group],["IBAN",`${SELLER_PROFILE.iban.slice(0,10)}...${SELLER_PROFILE.iban.slice(-4)}`]].map(([k,v],i)=>(
            <div key={i} style={{display:"flex",justifyContent:"space-between",fontSize:12,marginBottom:6}}><span style={{color:"#666"}}>{k}</span><span style={{color:"#fff",fontWeight:600}}>{v}</span></div>
          ))}
        </div>
        {doc.buyer&&<div style={{marginBottom:16,paddingBottom:16,borderBottom:"1px solid #2a2a2a"}}><div style={{fontSize:11,color:"#666",marginBottom:8,fontWeight:700}}>ПОКУПЕЦЬ</div><div style={{fontSize:13,color:"#fff",fontWeight:700}}>{doc.buyer}</div></div>}
        <div style={{marginBottom:16,paddingBottom:16,borderBottom:"2px dashed #2a2a2a"}}>
          <div style={{fontSize:11,color:"#666",marginBottom:8,fontWeight:700}}>РОЗРАХУНОК</div>
        ...
        <div style={{background:"#111",borderRadius:12,padding:12}}>
          ...
          {[["Система","Спрощена, 2 група"],["Ставка ЄП","₴1,600/міс (фіксована)"],["ПДВ","Не є платником"]].map(([k,v],i)=>(
            <div key={i} style={{display:"flex",justifyContent:"space-between",fontSize:11,marginBottom:4}}><span style={{color:"#666"}}>{k}</span><span style={{color:"#fbbf24",fontWeight:600}}>{v}</span></div>
          ))}
        </div>
      </div>
      <div style={{display:"flex",gap:10}}>
        <button style={{flex:1,padding:"12px 16px",background:"#1a1a1a",border:"1px solid #2a2a2a",borderRadius:14,color:"#888",fontSize:13,fontWeight:700,cursor:"pointer"}}>📤 Поділитись</button>
        <button style={{flex:1,padding:16,background:"#22c55e",border:"none",borderRadius:14,color:"#fff",fontSize:15,fontWeight:800,cursor:"pointer"}}>⬇️ PDF</button>
      </div>
    </div>
  );
}
    ...
    <div style={{padding:20}}>
      <div style={{background:"linear-gradient(135deg,#1a0d2e,#2d1b69)",borderRadius:20,padding:20,marginBottom:20}}>
        <div style={{display:"flex",gap:12,alignItems:"center",marginBottom:16}}>
          ...
          <div>
            <div style={{fontSize:16,fontWeight:900,color:"#fff"}}>{SELLER_PROFILE.fop}</div>
      ...
      <div style={{background:"#1a1a1a",borderRadius:16,padding:16,marginBottom:16}}>
        <div style={{fontSize:12,color:"#666",marginBottom:10,fontWeight:700}}>🏦 Банківський рахунок</div>
        <div style={{fontSize:12,color:"#fff",fontWeight:700,marginBottom:4}}>{SELLER_PROFILE.bank}</div>
        <div style={{fontSize:11,color:"#60a5fa",fontFamily:"monospace"}}>{SELLER_PROFILE.iban}</div>
        <div style={{marginTop:10,display:"flex",gap:8}}>
          <span style={{background:"#052e16",color:"#22c55e",fontSize:10,padding:"3px 8px",borderRadius:8,fontWeight:700}}>✓ Верифіковано</span>
          <span style={{background:"#0d1a2e",color:"#60a5fa",fontSize:10,padding:"3px 8px",borderRadius:8,fontWeight:700}}>UAH рахунок</span>
        </div>
      </div>
      <div style={{background:"#1a1a1a",borderRadius:16,padding:16,marginBottom:20}}>
        <div style={{fontSize:12,color:"#666",marginBottom:12,fontWeight:700}}>📊 Податкові зобов'язання</div>
        {[["📅","Єдиний податок","₴1,600 · до 20-го щомісяця","pending"],["💼","ЄСВ","₴1,430 · щоквартально","ok"],["📋","Звітність","1 раз на рік · до 01.03","ok"],["📄","РРО/ПРРО","Не потрібен (2 група)","ok"]].map(([ic,l,v,s],i)=>(
          <div key={i} style={{display:"flex",gap:10,alignItems:"center",padding:"10px 0",borderBottom:i<3?"1px solid #222":"none"}}>
            ...
            <div style={{flex:1}}>
              <div style={{fontSize:12,fontWeight:700,color:"#fff"}}>{l}</div>
              <div style={{fontSize:11,color:"#666"}}>{v}</div>
            </div>
            <span style={{fontSize:16}}>{s==="pending"?"⚠️":"✅"}</span>
          </div>
        ))}
      </div>
      <button onClick={onSetup} style={{width:"100%",padding:"12px 16px",background:"#1a1a1a",border:"1px solid #2a2a2a",borderRadius:14,color:"#888",fontSize:13,fontWeight:700,cursor:"pointer"}}>✏️ Оновити дані ФОП</button>
    </div>
  );
}
...
function WalletSection() {
  ...
  return (
    <div>
      {!page&&(
        <div style={{background:"#0f0f0f",padding:"16px 20px 12px",borderBottom:"1px solid #1a1a1a"}}>
          <div style={{display:"flex",alignItems:"center",gap:10}}>
            <div style={{width:40,height:40,background:"#1a1a1a",borderRadius:12,display:"flex",alignItems:"center",justifyContent:"center",fontSize:22}}>{SELLER_PROFILE.avatar}</div>
            <div>
              <div style={{fontSize:15,fontWeight:800,color:"#fff"}}>{SELLER_PROFILE.name}</div>
              <div style={{display:"flex",alignItems:"center",gap:6}}>
                <div style={{width:6,height:6,background:"#22c55e",borderRadius:"50%"}}/>
                <span style={{fontSize:10,color:"#22c55e",fontWeight:700}}>ФОП · Верифіковано</span>
              </div>
            </div>
            <div style={{marginLeft:"auto",background:"#1a1a1a",borderRadius:10,padding:"6px 10px",fontSize:11,fontWeight:700,color:"#fbbf24"}}>⚠️ Сплатити ЄП</div>
          </div>
        </div>
      )}
